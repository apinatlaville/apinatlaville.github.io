...
console.log('\n[5] Arbre Base Doc — collapse, mode, carte');
{
  ...
  assert(!html.includes('Cinématique'), 'replié : pas de carte');
  ...
  assert(html.includes('Cinématique'), 'carte après expand');
...
console.log('\n[7] Cache bump + boot-loader');
{
  ...
  assert(/__BOOT_CACHE_V\s*=\s*'20260721[a-z]'/.test(index), 'cache version bumpée');
